...
if (compsize > 3) {
  ...
  par(mfrow=c(sdim2,sdim1))
  for (node in c(1:nodes)){
    dummy <- matrix(data=NA,nrow=length(lonvecM4),ncol=length(latvecM4))
    dummy[seaindsM4]<-codes_out4[node,]
    filled.contour3(x=lonvecM4, y=rev(latvecM4), dummy, xlim=c(120,240), ylim=c(60,20), zlim=c(-2.5,2.5), nlevels=10, col = rev(brewer.pal(n=10,name='RdBu')), main=paste('Var 4, node', node))  #this works, but doesn't mask out the land
  }
  par(new = "TRUE",plt = c(0.85,0.9,0.25,0.85),las = 1,cex.axis = 1)
  filled.legend(x=lonvecM4, y=rev(latvecM4), dummy, xlim=c(120,240), ylim=c(60,20), zlim=c(-2.5,2.5), nlevels=10, col = rev(brewer.pal(n=10,name='RdBu')))  
}
